...
namespace PocketBaseClient.SampleApp.Models
{
    public partial class Post : ItemBase
    {
        ...
        #region GetById
        public static Post? GetById(string id, bool forceLoad = false) 
            => DataServiceBase.GetCollection<Post>()!.GetById(id, forceLoad);

        public static async Task<Post?> GetByIdAsync(string id, bool forceLoad = false)
            => await DataServiceBase.GetCollection<Post>()!.GetByIdAsync(id, forceLoad);
        #endregion GetById
    }
}
